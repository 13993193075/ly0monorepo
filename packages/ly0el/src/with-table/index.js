...
const refresh = async ({scopeThis, noMessage}) => {
    ...
    if(result.code === 0){
        ly0utils.deepClone.deepMerge(scopeThis.tableData, {
            data: result.data,
            total: result.total,
            sort: scopeThis.query.sort,
...
const reload = async ({scopeThis}) => {
    ly0utils.deepClone.replaceObject(
        scopeThis.query,
...
const popupFind = async ({scopeThis}) => {
    ly0utils.deepClone.replaceObject(scopeThis.formData, scopeThis.query.formData)
    ...
    ly0utils.deepClone.replaceObject(scopeThis.formProps, scopeThis.find.formProps)
    // 弹出窗口
    ly0utils.deepClone.deepMerge(
...
const popupInsertOne = async ({scopeThis}) => {
    ly0utils.deepClone.replaceObject(scopeThis.formData, scopeThis.insertOne.formData)
    ly0utils.deepClone.replaceObject(scopeThis.formProps, scopeThis.insertOne.formProps)
    // 弹出窗口
    ly0utils.deepClone.deepMerge(
        scopeThis.formProps.popup,
        {visible: true}
...
const popupUpdateOne = async ({scopeThis, row}) => {
    ly0utils.deepClone.replaceObject(scopeThis.formData, row) // 继承行记录的值
    ly0utils.deepClone.replaceObject(scopeThis.formProps, scopeThis.updateOne.formProps)
    // 弹出窗口
    ly0utils.deepClone.deepMerge(
        scopeThis.formProps.popup,
        {visible: true}
...
const popupDoc = async ({scopeThis, row}) => {
    ly0utils.deepClone.replaceObject(scopeThis.formData, row) // 继承行记录的值
    ly0utils.deepClone.replaceObject(scopeThis.formProps, scopeThis.doc.formProps)
    // 弹出窗口
    ly0utils.deepClone.deepMerge(
        scopeThis.formProps.popup,
        {visible: true}
...
const submitFind = async ({scopeThis}) => {
    ly0utils.deepClone.replaceObject(scopeThis.query.formData, scopeThis.formData)
