...
function reset(scopeThis) {
  return new Promise((resolve, reject) => {
    getData(scopeThis).then(function (result) {
      scopeThis.data.carwithin_rec = result.data.carwithin_rec
      scopeThis.data.carpark = JSON.parse(JSON.stringify(result.data.carpark))
      dateReset(scopeThis)
...
function getData(scopeThis) {
  return new Promise(function (resolve, reject) {
    ...
    dataRequest
      .storpro({
        scopeThis,
        storproName: 'ly0d11.echart.echart0',
        data: {
...
function showEcharts(scopeThis) {
  ...
  for (let iCarpark = 0; iCarpark < scopeThis.data.carpark.length; iCarpark++) {
    option_xAxis_data = []
    ...
    for (let iDate = 0; iDate < scopeThis.arrDate.length; iDate++) {
      ...
      let dataFilter = scopeThis.data.carwithin_rec.filter(function (i) {
        // 时段排除
        if (
          scopeThis.arrDate[iDate].dateFrom &&
        ...
        // 停车场排除
        if (scopeThis.data.carpark[iCarpark]._id !== i.id_carpark) {
          return false
        }
      ...
      option_series[0].data.push(deal)
    }
    ...
    setTimeout(function () {
      let app = echarts.init(document.getElementById('echarts-show' + iCarpark))
