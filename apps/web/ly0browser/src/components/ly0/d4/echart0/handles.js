...
const days = blindboxes.dateFormat.days
...
function collapseAll({scopeThis}) {
    ...
    scopeThis.data.hotel.forEach((hotel, iHotel) => {
        ...
        scopeThis.arrDate.forEach((d, iDate) => {
            ...
            scopeThis.data.goods.forEach((goods, iGoods) => {
                scopeThis.collapseOpen.push('hotel.' + hotel._id + '-date.' + iDate + '-goods.' + goods._id)
            })
            scopeThis.collapseOpen.push('hotel.' + hotel._id + '-date.' + iDate + '-goods.unknown')
        })
    })
}
