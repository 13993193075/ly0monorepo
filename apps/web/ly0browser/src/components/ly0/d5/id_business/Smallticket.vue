<template>
  ...
  >
    <el-divider content-position="center" class="title-line"
      ><span class="title-font">{{ arrPrinter.length > 0 ? '选择打印机' : '没有打印机' }}</span>
    </el-divider>
    <div v-for="(item, index) in arrPrinter" :key="index">
      <el-checkbox v-model="item.checked" style="margin-bottom: 20px"
        >{{ item.busiunit_name + '/' + item.printername + '/' + item.sceneNote }}
      </el-checkbox>
    </div>
      ...
      this.$confirm('打印?', '警告', {
        ...
        .then(() => {
          ...
          dataRequest
            .storpro({
              scopeThis: this,
              storproName: 'ly0d5.smallticket.print',
              data: {
    ...
    popup(valNew, valOld) {
      if (valNew) {
        dataRequest
          .storpro({
            scopeThis: this,
            storproName: 'ly0d5.smallticket.getPrinters',
            data: { id_business: this.scopeThis.business.objBusiness._id },
          })
          .then((result) => {
