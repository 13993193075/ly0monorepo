<template>
  <div style="padding: 10px">
    ...
    ></compFormUpdateOne>
    <compQrcode v-if="arrSelection.length > 0" :scopeThis="scopeThis"></compQrcode>
    <compWxacode v-if="arrSelection.length > 0" :scopeThis="scopeThis"></compWxacode>
  </div>
</template>
...
import compQrcode from './qrcode.vue'
import compWxacode from './wxacode.vue'
// 数据表属性
    ...
    return {
      ...
      pageData: {
        queryBody: {
          ...
          id_restaurant: fieldsValue_init.ly0session.user.id_restaurant
            ? fieldsValue_init.ly0session.user.id_restaurant
            : null,
        },
        data: {
          arrRestaurant: [],
