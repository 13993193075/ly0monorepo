function hotelChange({scopeThis}){
    // 级联 - 客房分区
    scopeThis.pgData.data.arrRoomplace0 = scopeThis.pgData.data.arrRoomplace.filter(i => {
        return i.id_hotel === scopeThis.formData.id_hotel
    })
    // 级联 - 房型
    scopeThis.pgData.data.arrGoods0 = scopeThis.pgData.data.arrGoods.filter(i => {
        return i.id_hotel === scopeThis.formData.id_hotel
    })
}
