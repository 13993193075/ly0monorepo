...
function queryRevise(data) {
    return new Promise((resolve, reject) => {
        ...
        // 收货人手机号
        if (data0.consignee_cellphone) {
            data1.consignee_cellphone = data0.consignee_cellphone
        }
        // 收货人姓名 模糊匹配
        if (data0.consignee_name) {
            data1.consignee_name = {'$regex': `.*${data0.consignee_name}.*`}
        }
...
function find(data) {
    ...
    // data.query.consignee_cellphone
    // data.query.consignee_name
    // data.sort.label
    // data.sort.order
    // data.limit
    // data.page
...
function dataRule(data) {
    ...
    if (!data.consignee_cellphone) {
        return {code: 1, message: "收货人手机号：必填项"}
    }
    if (!data.consignee_name) {
        return {code: 1, message: "收货人姓名：必填项"}
    }
                    ...
                    GQuery({
                        ...
                        GQuery({
                            ...
                            update: {
                                ...
                                count: data.count,
                                time: data.time,
                                id_goodsto: objGoodsto ? objGoodsto._id : null,
                                goodsto_text: objGoodsto ? objGoodsto.text : "",
                                keeper_cellphone: data.keeper_cellphone,
                                keeper_name: data.keeper_name,
                                consignee_cellphone: data.consignee_cellphone,
