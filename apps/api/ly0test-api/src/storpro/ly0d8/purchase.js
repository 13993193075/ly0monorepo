...
function queryRevise(data) {
    return new Promise((resolve, reject) => {
        ...
        // 货品来源
        if (data0.id_goodsfrom) {
            data1.id_goodsfrom = data0.id_goodsfrom
        }
...
function dataRule(data) {
    ...
    if (!/^[0-9]+$/.test(data.price)) {
        return {code: 1, message: "采购单价：必填项，数值，>= 0"}
    }
    if (!data.time) {
        return {code: 1, message: "入库时间：必填项"}
    }
            ...
            GQuery({
                ...
                GQuery({
                    ...
                    GQuery({
                        ...
                        operator: "findOne",
                        query: {_id: data.id_goodsfrom ? data.id_goodsfrom : null}
                    }).then(result=>{
                        let objGoodsfrom = result.data
                        GQuery({
                            tblName: "ly0d8purchase",
                            operator: "insertOne",
                            update: {
                                ...
                                price: data.price,
                                time: data.time,
                                id_goodsfrom: data.id_goodsfrom ? objGoodsfrom._id : null,
                                goodsfrom_text: data.id_goodsfrom ? objGoodsfrom.text : "",
